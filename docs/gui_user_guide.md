...
2. **Detection** — reconstructed frames are passed through a **YOLOv8n** model trained on the FRED dataset. Results are cached to disk and visualised here.
...
| `hypere2vid` | 8002 | Placeholder (not yet implemented) |
...
| **mAP50** | Validation accuracy of the current YOLO weights (Run 4: 63.4%) |
...
Shows the E2VID-reconstructed frames side-by-side (E2VID left, HyperE2VID right — the right panel shows a placeholder until HyperE2VID is implemented).
...
- **e2vid + YOLO** — currently functional
- **HyperE2VID + YOLO** — not yet implemented
...
- **Subsequent visits**: the cache is loaded automatically as soon as you switch to the Detection tab. No button press needed. The status line shows `Cached — N detections across all frames`.
...
One file per sequence: `detections_e2vid.json` stored in the processed data directory (`RECON_DATA_PATH/sequence_XX/`). It contains every detection with confidence ≥ 0.1, regardless of the slider setting. The confidence slider is a pure display filter and never invalidates the cache.

### When to re-run detection

- After training a new YOLO model and copying the new weights into the container.
...
Roughly 1–2 MB per sequence (5 sequences ≈ 8 MB total). Very small; suitable for baking into the container image for offline demonstration.

---

## Comparison

Side-by-side view of all three pipelines (E2VID, HyperE2VID, Late Fusion). Playback is synchronised. **Not yet implemented** — shows placeholder panels.
...
| Table | What it shows |
|-------|---------------|
| **Detection accuracy** | mAP@0.5, mAP@0.5:95, Precision, Recall on the validation set (sequence_127). Rows from the FRED paper are shown for reference. |
...
Results from our runs (highlighted in blue) can be compared directly against the FRED paper baselines (grey rows).
...
- **Run Detection button not visible**: the model selection row may be pushing it off-screen. Scroll down in the Detection panel, or use a wider browser window.
- **"Could not reach e2vid service"**: the e2vid container is unhealthy. Check the Admin tab and restart the Docker stack.
